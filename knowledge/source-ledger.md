# Source Ledger — Bootstrap Research

Every non-trivial claim in `spec.md`, `plan.md`, `DECISIONS.md`, `docs/`, and `knowledge/` should trace to a source listed here, to a local file/commit read in-session, or be marked `[assumed]`. Format: title — exact URL/file/command — access date — type — claims supported — reliability — notes.
...
| `test/` tree (16,961 LOC; 38 files: 37 `.js` incl. `support/match.js` + `.eslintrc.json` — count proven by `tests/test-hash-manifest.json`) | listed/read selected 2026-07-31 | repo files | test families and behavior coverage; support/match.js semantics (returns matched outputs via returnObject) | primary |
...
| `test/options.js` (partial, lines 1–120) | read 2026-07-31 | repo file | matchBase windows behavior (4.0.5 fix), flags/nocase, noextglob, unescape, nonegate, windows defaults | primary |
| `.github/workflows/test.yml` | read 2026-07-31 | repo file | CI matrix node 12/14/16/18/20/22/24/25 × ubuntu/windows/macos (macos excludes node 12/14); actions/checkout@v6 pin de0fac2e, setup-node@v6 pin 53b8394; npm install; npm run test:ci; coverage upload only ubuntu+node25 | primary |
...
| `CHANGELOG.md` | read 2026-07-31 | repo file | history to 4.0.0 only; 4.0.1–4.0.5 omitted (documentation gap) | primary |
| `LICENSE` | read 2026-07-31 | repo file | MIT, copyright 2017–present Jon Schlinkert | primary |
...
| `node tools/research/probe-regex-sources.js` → `scratch/probe-regex-sources.json` | 2026-07-31 01:54 | command | exact makeRe sources for ~60 pattern/option combos; error strings; match samples (windows backslash, negation, dot) | primary |
| `node -e` probes (API shapes) | 2026-07-31 01:51 | command | exported keys (9); parse state keys incl. peek/advance; matcher state keys {negated,fastpaths,output}; scan tokens JSON (Infinity→null); result object keys; posix entry !== index entry on windows | primary |
| `node tools/research/redos-timing.js` (bounded timing probe) | 2026-07-31 19:50 | command | `+(ab|abab)` NOT literalized on 4.0.5; median-of-5 curve 0.1/0.7/4.9/13.5/34.4/88.6/147.6 ms @ n=20–35 (len 41–71) — super-linear growth confirmed; supersedes the unreproducible 963 ms single point from 02:12 | primary |

## Primary: web (accessed 2026-07-31)

| Source | URL | Type | Claims supported | Reliability |
|---|---|---|---|---|
| Port Mortem 2026 — Devfolio page | https://portmortem.devfolio.co/ | official event | tracks (JS→Go or Rust), judging 40/30/20/10, submission artifacts, rules (single command, no source-runtime wrappers, preserve tests, license, plagiarism), timeline, $1,800, Discord exclusivity | primary (official) |
| Port Mortem — official event site | https://coderesurrection.com/2026 | official event | kickoff Jul 31 18:00 UTC; freeze Aug 03 18:00 UTC; test hashing at kickoff; thin-adapter test model; scoring details; bonus table (+5/+5/+3/+3); anatomy layout incl. .port-mortem.toml; flaky-test handling; 60s fuzz bonus; no pre-kickoff code; evaluation protocol; judges; Write-Up side quest $300. NOTE (corrected 2026-07-31): the "repo pool ~30, ≥80% coverage, 4h pick window" details originally recorded here are NOT on the current page — superseded by the pool-page row below | primary (official) |
| **Recommended Repo Pool — Port Mortem** | https://coderesurrection.com/2026/repo-pool | official event | **eligibility: pool lists micromatch/picomatch, JavaScript → Go,Rust, 2000-2500 LOC, Standard (2k–8k), MIT** (fetched 2026-07-31, footer v2026.07); pool ~104 repos across 8 tracks; BYO rules: ≥1,000 LOC, <~8,000, OSS, "not already ported to the target", no pre-approval; "tests are optional… passing the original suite unmodified is the ideal" | primary (official) |
...
| picomatch issue #171 | https://github.com/micromatch/picomatch/issues/171 | issue | parens not treated as literals — picomatch `/foo/(a)` matches `/foo/a`, bash 5.2 does not; open | primary (behavior divergence documented) |
...
| Bash Reference Manual — Pattern Matching | https://www.gnu.org/software/bash/manual/html_node/Pattern-Matching.html | manual | extglob operators, dotglob semantics, globstar ** meaning | primary |
...
| fancy-regex docs | https://docs.rs/crate/fancy-regex/latest , https://github.com/fancy-regex/fancy-regex | crate docs | backtracking VM over regex crate; lookaround/backrefs; linear only for non-fancy subset; **re-verified 2026-07-31: current 0.19.0** (0.17.0 was stale); `RegexBuilder::backtrack_limit` exists, **default 1_000_000** (lib.rs:617/747, raw source fetch 2026-07-31); `RuntimeError::{BacktrackLimitExceeded, StackOverflow}` (docs.rs 2026-07-31); registry license MIT, rust_version 1.66 | primary |
...
| releases.rs | https://releases.rs/ | version tracker | **re-fetched 2026-07-31: stable 1.97.1**, beta 1.98.0, nightly 1.99.0 (earlier note "1.95.0 stable" was stale by two releases) | secondary |
| crates.io regex | https://crates.io/api/v1/crates/regex | registry API | **re-verified 2026-07-31: latest 1.13.1** (published 2026-07-15; the "1.12.4" note was wrong on version, right on date); rust_version 1.65; license MIT OR Apache-2.0 | primary |
...
| devongovett/glob-match | https://github.com/devongovett/glob-match | repo | fast in-place glob matching, no regex, captures, brace nesting ≤10, cargo-fuzz present | primary |
...
- Upstream code: MIT © 2017–present Jon Schlinkert (local LICENSE).
- regex, regex-automata, fancy-regex, proptest, criterion: MIT or MIT/Apache-2.0 dual (registry pages above) — all compatible with MIT submission.
