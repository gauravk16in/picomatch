...
| Issue #175 (OPEN, third-party audit, not a CVE) | incomplete ReDoS fix | ≤4.0.5 (verified on our baseline) | — | `hasRepeatedCharPrefixOverlap` only catches single-char repeats; `+(ab|abab)` still compiles to `^(?:(?=.)(?:ab|abab)+)$` — verified locally 2026-07-31 with the bounded harness `tools/research/redos-timing.js` (input `'ab'×n+'c'`, 5 reps, median): 0.1/0.7/4.9/13.5/34.4/88.6/**147.6 ms** at n=20/24/28/30/32/34/35 — super-linear growth confirmed; the earlier single-point "963 ms @ len 71" was NOT reproducible on this machine (median 147.6 ms, spread 135–176; issue's own table says 134 ms on M1) and is withdrawn as evidence. |
...
| `+(ab|abab)` (issue #175) | **compiles to catastrophic regex** (baseline) | reproduce oracle behavior (D-011); document; fallback engine explicit `backtrack_limit` bounds OUR worst case with a typed `ResourceLimitError` (D-003, DV-6); Bug Catcher report | D-011, NFR-004 |
| Catastrophic backtracking generally | JS engine exponential on some non-risky lookahead patterns | primary engine linear-time; fallback explicit `backtrack_limit` with typed `ResourceLimitError` on trip (D-003, DV-6) — never silent no-match | FR-073 |
...
1. Issue #175: add our reproduction evidence as a comment candidate on the existing issue (no duplicate filing). Evidence protocol (2026-07-31, closes readiness-review F-17): committed harness `tools/research/redos-timing.js` — pattern `+(ab|abab)`, input `'ab'.repeat(n)+'c'` (matches the issue PoC), 5 repetitions per length, median + spread, environment recorded (Node v24.13.0, win32, i7-12650H). Measured: medians 0.1/0.7/4.9/13.5/34.4/88.6/147.6 ms at n=20/24/28/30/32/34/35 (len 41–71) — super-linear (roughly doubling per +2..3 chars). Single-point figures are not cited; any filed comment uses the median table + harness for reproducibility. Phase 9 may extend lengths only under a worker-thread subprocess timeout (never unbounded). Owner: team, Phase 9.
