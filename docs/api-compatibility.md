...
| `picomatch.parse(pattern, options)` | `parse(pattern, options) -> Result<ParseState>` | P0 (data fields) | fastpaths always false; state incl. `tokens`; function fields (`peek`, `advance`) not ported (G-06); numeric fields are UTF-16 code-unit offsets (D-013) |
| `picomatch.scan(input, options)` | `scan(input, options) -> ScanState` | P0 | full field contract + tokens/parts/maxDepth; `start`/`slashes` are UTF-16 code-unit offsets (D-013) `[lib/scan.js:327-388]` |
