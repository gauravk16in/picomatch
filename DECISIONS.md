...
Lightweight ADRs. Fields: status (Proposed/Accepted/Superseded), date, context, options, decision, evidence, consequences, rejected alternatives, reversal trigger, links. Judges read this â€” entries must be non-trivial and evidence-backed (â‰¥10 targets the Decision Log bonus).

## D-001 â€” Product shape: pure Rust library + differential CLI; N-API only as stretch

- Status: **Proposed** (requires team ratification at Phase 0 gate)
- Date: 2026-07-31
- Context: Event allows any artifact that builds with one command and runs the original tests via a thin adapter `(verified: coderesurrection.com/2026 Â§04/FAQ)`. Team has not formally chosen the shipping surface.
...
- Consequences: simplest single-command build (`cargo build --release`); adapter fidelity depends on CLI protocol completeness (spec Â§16, docs/differential-testing.md Â§5).
- Rejected: (b) as core â€” packaging/ABI risk inside 72h; (c) WASM â€” perf + toolchain risk; (d) CLI-only â€” loses library judging points for idiomatic Rust API.
- Reversal trigger: Discord template mandates in-process JS loading, or team vote at Phase 0.
- Links: plan Phase 0/2/8; spec Â§3, Â§17; context.md A-1.

## D-002 â€” Parity boundary: behavioral parity MUST; regex-source parity STRETCH with normalization

- Status: **Proposed** (ratify at Phase 0)
- Date: 2026-07-31
- Context: JS `makeRe().source` strings embed lookarounds `(?!.)`, `(?=.)`, `^(?!...).*$` that Rust `regex` cannot compile `(verified: docs.rs/regex)`. Exact string parity is therefore impossible without a custom regex engine for the whole language (huge scope) â€” but behavior parity is fully achievable.
...
- Decision: (a). Corpus records behavior always; `compareSource` recorded only where identical strings are achievable (literal-heavy patterns). (c) is adopted as presentation detail: we MAY retain the JS-form source string as metadata while executing a translated Rust-regex â€” documented, not hidden.
- Evidence: probe corpus (scratch/probe-regex-sources.json â€” ~60 source strings, most contain lookarounds); regex docs (no lookaround); research-gaps G-05.
- Consequences: honest scope; Behavior Equivalence judging is served by differential proof, not string diffs.
- Rejected: (b) â€” ReDoS re-introduction + schedule risk.
- Reversal trigger: judges publish a rubric explicitly scoring `makeRe` source equality (none found in research).
- Links: spec Â§5.4; docs/rust-design-options.md; DECISIONS D-003/D-004.

## D-003 â€” Two-engine architecture: `regex` primary, guarded fallback for lookaround subset
...
- Evidence: fancy-regex docs (VM delegates to regex when non-fancy; exponential worst case possible); regex-automata docs (no lookaround anywhere); Search MCP research #61 (hybrid recommended); GHSA-c2c7-rcm5-vvqj (backtracking danger â€” mitigated by step budget + risky-extglob literalization parity).
...
- Rejected: (1) â€” semantically impossible for `!(...)`; (2) â€” no lookaround; full custom VM â€” ReDoS + complexity.
- Reversal trigger: Phase 5/7 differential corpus shows systematic fancy-regex mismatch; or measured performance unacceptable â†’ substitute (4).
- Links: spec Â§11 (FR-070..074); docs/rust-design-options.md; plan Phase 5/7/9.

## D-004 â€” Compile strategy: transliterate parse.js 1:1 to a Rust IR emitting engine-specific sources
...
- Decision: (a) for the parser/tokenizer (with Rust-idiomatic boundaries: enums, no nulls, slices not mutation of shared string), then a thin emission layer translating the JS-source string into engine-appropriate regex (identity translation for fallback; lookaround-eliminating translation where a documented equivalence exists for the primary engine, e.g. `(?!.)`+`(?=.)` guards folded into segment classes where provably equivalent â€” otherwise route to fallback).
...
- Rejected: (b) â€” drift risk; (c) â€” the language is context-sensitive (brace/extglob state), grammars fight it.
- Reversal trigger: corpus divergence density > 2% in Phase 5 â†’ revisit (b) for offending constructs only.
- Links: spec Â§10; docs/parser-and-scanner.md; ARCHITECTURE.md.

## D-005 â€” Dependency policy
...
- Rejected: "batteries included" crates (glob/globset/glob-match) as semantic engine â€” proven incompatible with picomatch semantics (comparators only).
- Reversal trigger: any dep shows advisory or behavior mismatch â†’ replace per evaluation doc.
- Links: knowledge/dependency-evaluation.md; docs/licensing.md.

## D-006 â€” Differential oracle design: JS only in dev tooling, JSONL corpus, named-function registry
...
- Links: docs/differential-testing.md; spec Â§16; plan Phase 1.

## D-007 â€” Fuzzing strategy: proptest grammar + cargo-fuzz on Linux CI; 60s zero-divergence gate

- Status: **Accepted** (bootstrap architecture)
- Date: 2026-07-31
- Decision: three fuzz targets (scan/parse/match) with structure-aware arbitrary inputs; seeds from upstream suite + malicious cases; proptest for differential properties with committed regression files; â‰¥60s zero-divergence run published for the bonus. Windows dev machines can't run cargo-fuzz â†’ Linux CI job owns fuzzing.
...
## D-008 â€” Benchmark methodology: shared workload, distributions, no cherry-picking
...
- Evidence: event Â§09 scoring ("Throughput-only benchmarks score below honest p99 regressions"); criterion baseline docs.
- Links: docs/benchmarking.md; plan Phase 10.

## D-009 â€” Unsafe policy: forbid by default; any exception needs per-block justification + Miri
...
## D-010 â€” Prior-art non-copy policy and independent-derivation evidence
...
## D-011 â€” Reproduce-or-document for upstream bugs (issue #175 class)
...
- Decision: match the oracle behavior exactly (do NOT literalize `+(ab|abab)`), document the hazard in docs/security.md, and prepare a Bug Catcher report referencing existing issue #175 (add reproduction evidence; no duplicate filing). If maintainers fix it mid-hackathon, follow the fork HEAD â€” do not cherry-pick.
...
- Reversal trigger: regex 1.12.x requires newer â†’ bump pin, record here.
