...
- **Mutation-tested canaries:** 67 canaries (validator mutations + real-CLI canaries + stats synthetic fixture), all importing the production validator

## Correctness Evidence

- `cargo test`: 66 unit + 2 integration, 0 failed
- `cargo fmt --check` / `cargo clippy -D warnings`: clean
- `node fixtures/run-integrated.js`: 16/16 steps green (includes scanner corpus 3,216/3,216 deterministic, attack-scan 4,932 comparisons/0 divergences, attack-integrated 4,189/0, unchanged-original-test bridge 40/40, 19 harness canaries, 67 benchmark canaries, original-test hash pin)
