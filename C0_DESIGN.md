...
**[I-1] `Parser` embeds `state` wholesale** — no duplicated scalar field list, no `finish()` reassembly. `Parser { state: ParseState, input_chars: Vec<u16>, prev: usize, extglobs: Vec<ExtglobFrame>, stack: Vec<CounterKind>, platform: &'static PlatformChars, extglob_chars: &'static ExtglobChars, fragments: Fragments, opts: Options }`. `finish()` = recovery → maybe_slash → rebuild → `Ok(self.state)`. *(Impl deviation, declared post-review c0-1: `braces: Vec<BraceFrame>` deferred to C5 with the frame type it needs; `len: isize` became a method, `isize::try_from`, not a stored field. Functionally inert in C0.)*
