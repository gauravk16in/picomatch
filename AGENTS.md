...
Gates (current): cargo fmt/clippy `-D warnings`/test (66 unit + 2 integration) green; `node fixtures/run-integrated.js` 16/16 steps green; `node benchmarks/bench-canaries.js` 67/67; `node benchmarks/verify-artifacts.js benchmarks/results` verifies the E2 final set.
