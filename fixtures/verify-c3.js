...
/**
 * C3 corpus verifier — re-drives every c3 case against the reference and
 * byte-compares. node fixtures/verify-c3.js
 */
...
const REF = process.env.PICOMATCH_REF || path.join(__dirname, '..', '..', 'picomatch');
const parse = require(path.join(REF, 'lib', 'parse'));
const { canonState, decOptions } = require('./canon');

const corpusPath = path.join(__dirname, 'c3_oracle.json');
...
console.log(`verify-c3: ${pass}/${doc.cases.length} cases deterministic, corpus sha256 ${sha.slice(0, 12)}…`);
